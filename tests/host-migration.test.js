...
const compatibility = Object.freeze({
  ...
  squadSynergies: true, sharedParticipationCredit: true, downedActivity: true, joinInProgressNormalization: true, squadEnemyDirector: true, mapMechanics: true, campaignMutations: true, specialistMastery: true, rareDiscoveries: true,
  registryVersion: "lastlight.squad-synergy.v1", recoveryVersion: 12,
});
...
function checkpoint(overrides = {}) {
  return createMigrationCheckpoint({
    ...
    simulation: { version: 12, scalars: { tick: 120 } }, replay: { currentTick: 120 }, ...overrides,
  });
}
...
test("migration capabilities pin exact build, balance, runtime, and gameplay identity", () => {
  ...
  assert.equal(migrationCompatibilityMatches(compatibility, { ...compatibility, squadEnemyDirector: false }), false);
  assert.equal(migrationCompatibilityMatches(compatibility, { ...compatibility, mapMechanics: false }), false);
  assert.equal(migrationCompatibilityMatches(compatibility, { ...compatibility, campaignMutations: false }), false);
  assert.equal(migrationCompatibilityMatches(compatibility, { ...compatibility, rareDiscoveries: false }), false);
...
test("migration checkpoints use strict bounded anonymous state", () => {
  ...
  assert.throws(() => checkpoint({ simulation: { version: 8, scalars: { tick: 119 } } }), /match its tick/);
  assert.throws(() => checkpoint({ simulation: { version: 7, scalars: { tick: 120 } } }), /match its tick/);
...
test("migration readiness is bound to one epoch, checkpoint, tick, and hash", () => {
  ...
  assert.deepEqual(ready, {
    type: "migration_ready", schema: HOST_MIGRATION_SCHEMA, protocolVersion: 10,
